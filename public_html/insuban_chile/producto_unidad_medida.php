...
<?
if($id_p){
$sql="SELECT * FROM  cruce_plant_unidad_medida  WHERE id_producto = $id_p and id_cruce_plant_unidad_medida != 0 GROUP BY id_producto order by id_cruce_plant_unidad_medida  desc";
$result=mysql_query($sql);
$cuantos=mysql_num_rows($result);
}else{

$sql="SELECT * FROM  cruce_plant_unidad_medida  WHERE id_producto = id_producto and id_cruce_plant_unidad_medida != 0 GROUP BY id_producto order by id_cruce_plant_unidad_medida  desc";
$result=mysql_query($sql);
$cuantos=mysql_num_rows($result);


if(!$op) $op=0;
$sql="SELECT * FROM  cruce_plant_unidad_medida  WHERE id_producto = id_producto and id_cruce_plant_unidad_medida != 0 GROUP BY id_producto order by id_cruce_plant_unidad_medida  desc LIMIT $op , 1";
$result=mysql_query($sql);
$ante=$op - 1;
$next=$op + 1;
$ultimo=$cuantos - 1;
...
}echo"<meta http-equiv=\"refresh\" content=\"0;URL=sistema.php?modulo=producto_unidad_medida.php\">";
  exit;
}
...
<table width="455" height="100%" border="0" align="center">
  <tr>
    <td width="564" height="14" bgcolor="#CCCCCC" class="titulo">Asignar Unidades de Medidas </td>
    <td width="32" bgcolor="#CCCCCC" class="cajas"><a href="?modulo=producto_unidad_medida_listar.php">Volver</a></td>
  </tr>
  <tr>
...
$cuanrc=mysql_num_rows($rc);
		  ...
		  <table width="433" border="0" align="center">
              <tr>
                ...
                <td width="300"><? $um="um";
			$producto= crea_producto_onChange_solo($link,$rowi[id_producto],$um);
			echo $producto;
			?></td>
              </tr>
              <tr>
                <td colspan="2"><table width="319" border="0" align="center">
                    <tr>
                      <td width="171" nowrap="nowrap" class="titulo">Asignar  Unidad Medida </td>
        ...
        <tr>
          <td width="600" height="250"><table width="433" border="0" align="center">
            <tr>
              <td width="123" class="titulo">Producto</td>
              <td width="300"><? $um="um";
			$grupo_producto2= crea_producto($link,$id_producto,$um);
			echo $grupo_producto2;
			?></td>
            </tr>
            <tr>
              <td colspan="2"><table width="318" border="0" align="center">
                  <tr>
                    <td width="129" class="titulo">Crear Unidad Medida </td>
                    <td width="52"><input name="cant" type="text" id="cant" value="<? echo $cant?>" size="5" maxlength="5" /></td>
                    <td width="123"><label>
      ...
      <table width="335" border="0" align="center">
        <tr>
          ...
          <td><? if ($cuantos){ ?>
              ...
              <? }?>          </td>
          <td width="47"><? if($permiso19 == 1){?><a href="?modulo=producto_unidad_medida.php&amp;nuevo=1" ><img src="jpg/nuevo.jpg" width="47" height="13" border="0" /></a><? }?> </td>
          ...
          <td width="62"><? if($permiso19 == 1){?><? if(!$nuevo and $cuantos){?>
              ...
              <? }?><? }?></td>
          <td width="55"><? if($permiso19 == 1){?><? if(!$cuantos or $nuevo or $mantsec ){?>
              <a href="javascript: document.form1.submit();">
              <label>
              ...
              <? }?><? }?></td>
          <td width="45"><? if($permiso19 == 1){?><? if(!$nuevo and $cuantos){?>
              <a href="?modulo=producto_unidad_medida.php&amp;borrar=<? echo $id_bode?>" onclick='return Confirmar(this.form1)'> <img src="jpg/borrar.jpg" width="45" height="13" border="0" /> </a>
              <? }?> <? }?> </td>
