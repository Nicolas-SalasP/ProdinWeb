...
if(isset($_GET["barcode"])) $barcode=$_GET["cantidad"];
...
function Barcode39 ($barcode, $width, $height, $quality, $format, $text)
{
    ...
    or die ("Cannot Initialize new GD image stream");
        $White = ImageColorAllocate ($im, 255, 255, 255);// cambia el color de fondo de la etiqueta
        ...
        $ActualWidth = (($NarrowBar * 6) + ($WideBar*3) + $QuietBar) * (strlen ($barcode)+2);//ubicacion del codigo de barra
        
        if (($NarrowBar == 0) || ($NarrowBar == $WideBar) || ($NarrowBar == $QuietBar) || ($WideBar == 0) || ($WideBar == $QuietBar) || ($QuietBar == 0))
        {
                ImageString ($im, 0, 0, 0, "Image is too small!", $Black);
                OutputImage ($im, $format, $quality);
                exit;
        }
        
        $CurrentBarX = (int)(($width - $ActualWidth) / 1.5); // cambia la posicion el numero de izq a derec
        $Color = $White;
