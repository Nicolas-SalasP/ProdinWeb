...
if ($enpic){
	...
	//echo "cuantos_consulta $result_consulta";
//codigo que almacena datos de venta
...
//$sql="INSERT INTO paking (folio_piking,id_paking_relacion,id_etiquetados_folios,fecha_ingreso_paking) values ($folio_pinking2,$idxx,$id_etiquetados_folios,'$fecha_ingreso_paking')";
      ...
      if(!$cuantos_consulta)
	  {
		    ...
		    $sq_up="UPDATE etiquetados_folios set id_estado_folio = '3', fpicking ='$envio_pickhist' ,id_destinos='$id_destinos' where id_estado_folio != 3 and id_etiquetados_folios=$id_etiquetados_folios";
			$rest_up=mysql_query($sq_up);
			//echo "sql_up $sq_up<br>";
	  }//if(!$cuantos_consulta)
	}//while ($row=mysql_fetch_array($result))
			$fech_envio_picking222=date("Y-m-d");
		$sqenviopic="UPDATE pedido set folio_piking = '$folio_pinking2', id_paking_relacion='$idxx', fech_envio_picking ='$fech_envio_picking222'  where id_pedidos=$id_ped";
			$restenviopic=mysql_query($sqenviopic);
			//echo "sqenviopic $sqenviopic<br>";
 ...
 foreach ($_POST as $key => $value)
               {
				...
				$sql="UPDATE pedido_tabla SET cantidadb='$cantidadbc' where id_pedido_tablas = $id";
...
$sql_p="insert into pedido_tabla (id_pedidos,id_cruce_tablas,cantidadb,fecha_ingreso_producto_pedido,costo,venta, dolar, euro) values ('$id_pedidos','$codigo','$cantidadb','$fecha_ingreso_producto_pedido','$costo','$venta','$dolar','$euro')";
...
$sql2222="SELECT * FROM etiquetados_folios AS etiq, producto AS pro, cruce_tablas AS cr where etiq.id_etiquetados_folios = etiq.id_etiquetados_folios and etiq.id_estado_folio != 3 and etiq.id_estado_folio != 10 and etiq.id_estado_folio != 7 and etiq.id_estado_folio != 5 and etiq.id_estado_folio != 6 and etiq.id_estado_folio != 0  and etiq.id_estado_folio != 8 and etiq.id_estado_folio != 4 and etiq.id_producto = pro.id_producto and etiq.id_cruce_tablas = cr.id_cruce_tablas and etiq.id_pedidos = 0 and etiq.ocupado = 0 and etiq.ano >= 2008 and etiq.f_elaboracion  <= '$fec_resok' order by etiq.f_elaboracion asc";
//echo "ssssssssssssss1<br>";
//echo"<meta http-equiv=\"refresh\" content=\"0;URL=sistema.php?modulo=pedido_modificar.php&id_ped=$id_ped\">";

}else{
$sql2222="SELECT * FROM etiquetados_folios AS etiq, producto AS pro, cruce_tablas AS cr where etiq.id_etiquetados_folios = etiq.id_etiquetados_folios and etiq.id_estado_folio != 3 and etiq.id_estado_folio != 10 and etiq.id_estado_folio != 7 and etiq.id_estado_folio != 5 and etiq.id_estado_folio != 6 and etiq.id_estado_folio != 0 and etiq.id_estado_folio != 8 and etiq.id_estado_folio != 4 and etiq.id_producto = pro.id_producto and etiq.id_cruce_tablas = cr.id_cruce_tablas and etiq.id_pedidos = 0 and etiq.ocupado = 0 and etiq.ano >= 2008 order by etiq.f_elaboracion asc";
//echo"<meta http-equiv=\"refresh\" content=\"0;URL=sistema.php?modulo=pedido_modificar.php&id_ped=$id_ped\">";
...
<!--    <td width="473"><div align="right"><a href="?modulo=pedido_ingresar.php" class="titulo">Ingresar Nuevo Pedido</a></div>
    ...
    <td width="673"><div align="right"><a href="?modulo=pedido_listar_h.php" class="cajas">Volver Listado de Pedidos Anteriores</a></div></td>
  </tr>
  <tr>
            ...
            <table width="663" border="1" align="center" cellpadding="0" cellspacing="0">
              <tr>
                <td colspan="2" nowrap="nowrap" class="titulo">&nbsp;&nbsp;Cliente:<span class="cajas">
                  <?
				  echo "<font size='1' color='red'>$destinos</font>";
...
//				  $id_destinos=$row[id_destinos];
              ...
              <tr>
                <td colspan="2" nowrap="nowrap" class="titulo"><span class="cajas">&nbsp;</span></td>
          ...
          <table width="663" border="1" align="center" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td width="64" bgcolor="#CCCCCC" class="titulo"><div align="center">Costo Unid.</div></td>
                <td width="64" bgcolor="#CCCCCC" class="titulo"><div align="center">Venta Unid. </div></td>
...
<?}?>
			...
			<? } ?>
              <tr>
                <td height="23" colspan="4" nowrap="nowrap"><div align="left">&nbsp;&nbsp;&nbsp;<a href="?modulo=etiquetados_folios_listar_h.php&amp;id_ped=<? echo $id_pedidos?>&amp;id_destinosv=<? echo $id_destinos;?>&fecha_prioridad=<? echo $fecha_prioridad?>" class="cajas" >LISTAR DETALLE DE FOLIOS</a></div></td>
              ...
              <tr>
                ...
                <td height="23" colspan="5" nowrap="nowrap"><div align="center">
                   <!-- <input name="generar" type="submit" class="cajas" id="enpic2" value="Asignar Folios Automaticamente" /> -->&nbsp; <?echo $divisa;?> </div>
				</td>
              </tr>
	  ...
	  if($sumfila) {?>
        ...
        <input name="codigo" type="text" id="codigo" onblur="Verifica_datos();" value="<? echo $codigo;?>" size="4" maxlength="5" onkeypress="solo_numeros()"/></td>
                ...
                <td nowrap="nowrap" class="cajas"><input name="cantidadb" type="text" id="cantidadb" size="4" onkeypress="solo_numeros()"/>
                  &nbsp;</td>
                ...
                <td nowrap="nowrap" class="cajas"><input name="venta" type="text" id="venta" size="4" onkeypress="solo_numeros()"/>
                  &nbsp;</td>
				<td><input name="dolar" type="hidden" id="dolar" value="<? echo $dolar;?>"/></td>                
                <td><input name="euro" type="hidden" id="euro" value="<? echo $euro;?>"/></td>
              </tr>
              <tr>
                <td colspan="5" nowrap="nowrap"><div align="right" class="alarma">&nbsp;
					 ...
					 if(!$cuantosb and $codigo) { echo "El Codigo ingresado No Existe";}
						  ?>
                </div></td><td nowrap="nowrap"></td><td nowrap="nowrap"></td>
                <td nowrap="nowrap">&nbsp;
                    <? if($codigo){?>
...
$res_ped=mysql_query("SELECT * from observaciones where id_pedido = $id_pedidos ");
$resultp=mysql_fetch_array($res_ped);
$nota =$resultp[1];

echo "<br><br><center>";
echo "<font size='2' face='Tahoma'> Observaciones: $nota </font>";
echo "</center>"
