...
$pdf->ezImage("fri.png", 0, 70, 'none', 'left');
if ($id_cruce_tablas == 264) {
$pdf->ezImage("A.png", 0, 70, 'none', 'center');
}elseif ($id_cruce_tablas ==314 or $id_cruce_tablas ==706 ) {
$pdf->ezImage("B.png", 0, 70, 'none', 'center');
}elseif ($id_cruce_tablas == 506) {
$pdf->ezImage("C.png", 0, 70, 'none', 'center');
}elseif ($id_cruce_tablas == 649) {
$pdf->ezImage("D.png", 0, 70, 'none', 'center');
}
...
$pdf->ezText(utf8_decode("<b>Comprador: 	FRIMESA COOPERATIVA CENTRAL</b>"),9);
$pdf->ezText(utf8_decode("			             Medianeira - PR, 85884-000, Brasil. 
	CNPJ: 77.595.395/0002-28"),10);

$pdf->ezText('', 10);																
$pdf->ezText("<b>Bombona: $i / $cantifo</b> ",10);
...
$pdf->ezText('', 10);
if ($id_cruce_tablas == 264) {
	$pdf->ezText(utf8_decode("<b>961 - 32/36 6 FIOS</b>"),20);
}elseif ($id_cruce_tablas == 314) {
	$pdf->ezText(utf8_decode("<b>74328 - 32/36 8 FIOS</b>"),20);
}elseif ($id_cruce_tablas == 506) {
	$pdf->ezText(utf8_decode("<b>48226 - 40/42 CALABRESA</b>"),15);
}elseif ($id_cruce_tablas == 649) {
	$pdf->ezText(utf8_decode("<b>48226 - 40/42 CALABRESA</b>"),15);
}
