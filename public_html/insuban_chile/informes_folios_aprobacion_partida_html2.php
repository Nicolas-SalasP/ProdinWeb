...
$sqldes="SELECT * FROM destinos where id_destinos = $id_destinos"; 
	    ...
	    $cod_destino = $row_des[id_destinos];	
...
$cuanto_esp=mysql_num_rows($result_esp);
    ...
    <tr>
      <td colspan="2"><table width="660" border="1" align="center">
        <tr>
          <td width="71" class="texto_informe2">DESTINOS</td>
          <td width="77" class="texto_informe2">N&deg; PALLET </td>
          <td width="65" class="texto_informe2"> BULTOS </td>
...
$cuanto_esp=mysql_num_rows($result_esp);
	...
	?>
		...
		$sql_fec_elab="SELECT p.id_paking_relacion, ef.id_etiquetados_folios, esp.id_especie, esp.especie, ef.f_elaboracion, ef.f_inicio, ef.f_termino,  DATE_FORMAT( ef.f_elaboracion, '%M %Y' ) AS dateGroup
        FROM paking AS p, etiquetados_folios AS ef, especie AS esp 
        WHERE 
        p.id_paking_relacion = $id_paking_relacion
        AND p.id_etiquetados_folios = ef.id_etiquetados_folios
...
/*
    	...
    	$sql_fec_elab="SELECT p.id_paking_relacion, ef.id_etiquetados_folios, esp.id_especie, esp.especie, ef.f_elaboracion, ef.f_inicio, ef.f_termino,  DATE_FORMAT( ef.f_termino, '%M %Y' ) AS dateGroup
        FROM paking AS p, etiquetados_folios AS ef, especie AS esp 
        WHERE 
        p.id_paking_relacion = $id_paking_relacion
        AND p.id_etiquetados_folios = ef.id_etiquetados_folios
...
*/
	...
	$mess=split(" ",$row_fec_elab[f_elaboracion]);
...
//	$mess=split(" ",$row_fec_elab[f_termino]);
		...
		$sql_fec_ven="SELECT p.id_paking_relacion, ef.id_etiquetados_folios, esp.id_especie, esp.especie, ef.f_elaboracion, ef.f_inicio, ef.f_termino,  DATE_FORMAT( ef.f_elaboracion, '%M %Y' ) AS dateGroup
        FROM paking AS p, etiquetados_folios AS ef, especie AS esp 
        WHERE 
        p.id_paking_relacion = $id_paking_relacion
        AND p.id_etiquetados_folios = ef.id_etiquetados_folios
...
/*
    	$sql_fec_ven="SELECT p.id_paking_relacion, ef.id_etiquetados_folios, esp.id_especie, esp.especie, ef.f_elaboracion, ef.f_inicio, ef.f_termino,  DATE_FORMAT( ef.f_termino, '%M %Y' ) AS dateGroup
        FROM paking AS p, etiquetados_folios AS ef, especie AS esp 
        WHERE 
        p.id_paking_relacion = $id_paking_relacion
        AND p.id_etiquetados_folios = ef.id_etiquetados_folios
...
//		$dest= $row_fec_ven[id_destinos];		
	    $messs=split(" ",$row_fec_ven[f_elaboracion]);
//	    $messs=split(" ",$row_fec_ven[f_termino]);	    
 		  $messs=split("-",$messs[0]);
          $messs[2]-$messs[1]-$messs[0];
		  $dat111=$messs[1];
 	  	$dat222=$messs[0]+2; 	  
    ...
    <tr>
      <td colspan="2"><table width="650" border="1" align="center">
        ...
        <tr>
          ...
          <td class="texto_informe2"><div align="center">N&deg; BULTOS </div></td>
		...
		while ($row=mysql_fetch_array($rest)){
		$kilos =$row[kilos]; 
		$kilosf =number_format($kilos, 2, '.', '');


		if($kilos){
		?>    
		 <input name="kilos" type="text" class="cajas"   id="kilos"  value="<?echo $kilosf?>" size="10" maxlength="10" />
		<? 
		}else{ 
		...
		$sql_fec_resm="SELECT p.id_paking_relacion, ef.id_etiquetados_folios, esp.id_especie, esp.especie, ef.f_elaboracion, ef.f_inicio, ef.f_termino,  DATE_FORMAT( ef.f_elaboracion, '%M %Y' ) AS dateGroup
        FROM paking AS p, etiquetados_folios AS ef, especie AS esp 
        WHERE 
        p.id_paking_relacion = $id_paking_relacion
        AND p.id_etiquetados_folios = ef.id_etiquetados_folios
        ...
        ";
/*
    	$sql_fec_resm="SELECT p.id_paking_relacion, ef.id_etiquetados_folios, esp.id_especie, esp.especie, ef.f_elaboracion, ef.f_inicio, ef.f_termino,  DATE_FORMAT( ef.f_termino, '%M %Y' ) AS dateGroup
        FROM paking AS p, etiquetados_folios AS ef, especie AS esp 
        WHERE 
        p.id_paking_relacion = $id_paking_relacion
        AND p.id_etiquetados_folios = ef.id_etiquetados_folios
...
*/
    ...
    $mes_resm=split(" ",$row_fec_resm[f_elaboracion]);
//	$mes_resm=split(" ",$row_fec_resm[f_termino]);
		...
		?></div></td>
          <td width="120" valign="top" class="texto_informe1"><div align="left">
            <?


		$sql_fec_empaque="SELECT p.id_paking_relacion, ef.id_etiquetados_folios, esp.id_especie, esp.especie, ef.f_elaboracion, ef.f_inicio, ef.f_termino,  DATE_FORMAT( ef.f_elaboracion, '%M %Y' ) AS dateGroup
        FROM paking AS p, etiquetados_folios AS ef, especie AS esp 
        WHERE 
        p.id_paking_relacion = $id_paking_relacion
        AND p.id_etiquetados_folios = ef.id_etiquetados_folios
...
/*
    	$sql_fec_empaque="SELECT p.id_paking_relacion, ef.id_etiquetados_folios, esp.id_especie, esp.especie, ef.f_elaboracion, ef.f_inicio, ef.f_termino,  DATE_FORMAT( ef.f_termino, '%M %Y' ) AS dateGroup
        FROM paking AS p, etiquetados_folios AS ef, especie AS esp 
        WHERE 
        p.id_paking_relacion = $id_paking_relacion
        AND p.id_etiquetados_folios = ef.id_etiquetados_folios
...
*/
		...
		//echo "  FECHA $row_fec_resm[f_elaboracion] <br>";
      $mes_empaque=split(" ",$row_fec_empaque[f_elaboracion]);
//          $mes_empaque=split(" ",$row_fec_empaque[f_termino]);          
    ...
    <tr>
      <td colspan="2"><div align="center"><span class="texto_informe1">ORIGEN DE LA MATERIA PRIMA: &quot;ADJUNTO EN TRAZABILIDAD DEL PRODUCTO&quot;<br />
        Los productos y lotes cumplen con los requisitos sanitario del pa&iacute;s de destino<br />
              <span class="cajas">
              <? echo $pais;
		?>
