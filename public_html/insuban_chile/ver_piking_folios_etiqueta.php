...
<table width="616" border="0" align="center">
  ...
  <tr>
	...
	$clientes= crea_cliente($link,$id_dest,$id_etiqueta_idioma,1);
	echo $clientes;
	}
	?>
	 Desde
	 <input name="desde" type="text" id="desde" size="5" maxlength="5" value="<?echo $desde?>"/>
	 Hasta
	 <input name="hasta" type="text" id="hasta" size="5" maxlength="5"  value="<?echo $hasta?>"/>
	<? //if($id_etiquetas == 15 or $id_etiquetas == 7){
		if($id_dest and $id_etiqueta_idioma == 1){?>
		<div align="center">
	    <a href="javascript:Abrir_ventana('etiqueta_espanol.php?id_paking_relacion=<? echo $id_paking_relacion ?>&id_dest=<? echo $id_dest?>&bultos=<? echo $cuantos1 ?>&folio_piking=<? echo $row[folio_piking]?>&desde=<? echo $desde?>&hasta=<? echo $hasta?>&id_etiqueta_idioma=<? echo $id_etiqueta_idioma?>&id_dest =<? echo $id_dest ?>')"><img src="jpg/impresora.jpg" alt="Etiquetas" width="35" height="39" border="0" /><br>
	    Imprimir Etiquetas </a>	
		 </div>
	    <? }?>
		
		<? if($id_dest and $id_etiqueta_idioma == 2){?>
		<div align="center">
	    <a href="javascript:Abrir_ventana('etiqueta_ingles.php?id_paking_relacion=<? echo $id_paking_relacion ?>&bultos=<? echo $cuantos1 ?>&id_dest=<? echo $id_dest?>&folio_piking=<? echo $row[folio_piking]?>')"><img src="jpg/impresora.jpg" alt="Etiquetas" width="35" height="39" border="0" /><br>
	    Imprimir Etiquetas </a>	
		 </div>
	    <? }?>
		
		<? if($id_dest and $id_etiqueta_idioma == 3){?>
		<div align="center">
	    <a href="javascript:Abrir_ventana('etiqueta_portugues.php?id_paking_relacion=<? echo $id_paking_relacion ?>&bultos=<? echo $cuantos1 ?>&id_dest=<? echo $id_dest?>&folio_piking=<? echo $row[folio_piking]?>')"><img src="jpg/impresora.jpg" alt="Etiquetas" width="35" height="39" border="0" /><br>
	    Imprimir Etiquetas </a>	
		 </div>
	    <? }?>
	</td>
  ...
  <tr>
    <td width="614"><table width="614" border="1" align="center" cellpadding="0" cellspacing="0">
      ...
      <? 
		...
		$sql="SELECT pro.nombre AS prod, c.calibre, ef.contenido_unidades, ef.f_elaboracion, mp.nombre AS nombre3, ef.id_etiquetados_folios, ef.ano,ef.nombre_alt,ef.calibre_alt
      ...
      <tr>
		...
		echo $calibre;
		?>		</td>
        <td class="cajas"><?echo $r[nombre3]?></td>
