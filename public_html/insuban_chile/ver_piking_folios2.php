...
if ($modificar_datos)
  {
   ...
   //echo "gggggggggggggggg";
	...
	$id_estado_folio = 3;
		 ...
		 $sqlbuscapickingm="SELECT * from paking AS p, etiquetados_folios AS ef where p.id_paking_relacion = '$id_paking_relacion' and p.id_etiquetados_folios = ef.id_etiquetados_folios";
$restpickingm=mysql_query($sqlbuscapickingm);
	    while ($rowmuestra=mysql_fetch_array($restpickingm)){ 
				$id_etiquetados_foliosmuestra = $rowmuestra[id_etiquetados_folios];
				
				//echo "id_etiquetados_foliosmuestra $id_etiquetados_foliosmuestra<br>";
		

	
    $sq_up_piking="update paking set fdespacho_piking = '$fecha' where id_etiquetados_folios =$id_etiquetados_foliosmuestra";
	$rest_up_piking=mysql_query($sq_up_piking);
	$sq_up_etf="update etiquetados_folios set id_estado_folio = '$id_estado_folio', fdespacho_piking='$fecha' where id_etiquetados_folios =$id_etiquetados_foliosmuestra";
	$rest_up_etf=mysql_query($sq_up_etf);
   //}
   ...
   $f_despacho_picking1="$dat[2]-$dat[1]-$dat[0]";
   	...
   	$sq_up_etf="update etiquetados_folios set id_estado_folio = '$id_estado_folio', id_destinos='$id_destinos', factura='$factura_paking',guia='$guia',fdespacho_piking ='$f_despacho_picking1', glosa='$glosa'  where id_etiquetados_folios =$id_etiquetados_foliosmuestra";
	$rest_up_etf=mysql_query($sq_up_etf);
...
//echo "if($despachar_x and $id_estado_folio == 7 and $id_destinos){";
   ...
   $sqlbuscapicking="SELECT * from paking AS p, etiquetados_folios AS ef where p.id_paking_relacion = $id_paking_relacion and p.id_etiquetados_folios = ef.id_etiquetados_folios";
$restpicking=mysql_query($sqlbuscapicking);
//$cuantos1=mysql_num_rows($restpicking);

          while ($rpicking=mysql_fetch_array($restpicking)){ 
				$id_fo = $rpicking[id_etiquetados_folios];
	   	   
   
    //foreach ($id_etiquetados_folios as $key)
   //{
    $sq_up_piking="update paking set fdespacho_piking  = '$f_despacho_picking1' where id_etiquetados_folios =$id_fo";
	$rest_up_piking=mysql_query($sq_up_piking);
    //$fechafolio=date("Y-m-d");
	$sq_up_etf=" update etiquetados_folios set id_estado_folio = '7', id_destinos='$id_destinos', factura='$factura_paking',guia='$guia',fdespacho_piking ='$f_despacho_picking1', glosa='$glosa'  where id_etiquetados_folios =$id_fo ";
//echo "$sq_up_etf<br>";
	$rest_up_etf=mysql_query($sq_up_etf);
	//}
		  }
...
{ 
 ...
 $dat=split("-",$key); 
      ...
      if ($dat[0] == 'pallet')
   {
	...
	$sql="update etiquetados_folios set calibre_alt='',nombre_alt='', contenido_alt='' where pallet=$pallet";
    $rest=mysql_query($sql);
	//echo "SQL $sql<br>";
...
/*
$sql2="SELECT * FROM paking AS p, etiquetados_folios As ef, calibre AS c, producto AS pro, medidas_productos AS mp, caract_producto cpro where p.id_paking_relacion=$id_paking_relacion and ef.id_calibre=c.id_calibre and ef.id_producto = pro.id_producto	and	ef.id_medidas_productos = mp.id_medidas_productos and ef.id_caract_producto = cpro.id_caract_producto and ef.id_caract_producto = 25 and	p.id_etiquetados_folios= ef.id_etiquetados_folios group by ef.pallet order by ef.nombre_alt, ef.calibre_alt,mp.medidas_productos, ef.id_etiquetados_folios asc ";
...
<table width="623" border="0" align="center">
    ...
    <td width="307" nowrap="nowrap" class="titulo"><table width="306" border="1" cellpadding="0" cellspacing="0">
      <tr>
        <td height="185"><table width="295" border="0" align="center">
            ...
            <tr>
              <td colspan="3" align="center" class="titulo"><!--<input type="checkbox" name="chk" onClick="apply()">Muestras Sin Valor Comercial--></td>
            </tr>
        </table></td>
      </tr>
    </table></td>
 ...
 <!-- <tr>
   ...
   <tr>
	...
	<hr />
	<div align="center">
	  <input type="submit" name="despachar" id="despachar" value="Despachar Picking" />
	</div>
    <hr />	
    <? } ?>
    </td>
...
$sqlvsff="SELECT SUM(ef.contenido_unidades) AS contenido_unidadesp, SUM(ef.contenido_alt) AS contenido_altp,ef.id_etiquetados_folios AS id_etiquetados_folios, ef.id_cruce_tablas AS id_cruce_tablasv, ef.nombre_alt AS nombre_alt, ef.id_producto AS id_producto, pro.producto AS producto, ef.calibre_alt AS calibre_alt, c.calibre AS calibre, mp.medidas_productos AS medidas_productos, cpro.caract_producto AS caract_producto, ef.id_procedencia AS id_procedencia, ef.pallet AS pallet  FROM paking AS p, etiquetados_folios As ef, calibre AS c, producto AS pro, medidas_productos AS mp, caract_producto cpro where 
		...
		$restvsss=mysql_query($sqlvsff);
		$dfgdfgaaa=mysql_num_rows($restvsss);
		
		$dfgdfgaaa2=$dfgdfgaaa;
		
		$dfgdfgaaa="SELECT * FROM paking AS p, etiquetados_folios As ef, calibre AS c, producto AS pro, medidas_productos AS mp, caract_producto cpro where 
		p.id_paking_relacion=$id_paking_relacion
		and ef.id_calibre=c.id_calibre
		and ef.id_producto = pro.id_producto
		and	ef.id_medidas_productos = mp.id_medidas_productos
		...
		$restsddd=mysql_query($dfgdfgaaa);
		$cuantsadfos=mysql_num_rows($restsddd);
		$cuantsadfos2=$cuantsadfos;
			
		$ete22t=$cuantsadfos2 + $dfgdfgaaa2;
