...
if($id_procedencia == "I"){
	...
	if($id_mat_prima_nacional){	$daton=$id_mat_prima_nacional;	
	 $largo=strlen($id_mat_prima_nacional);
  	 if($largo == 10){ $agr=2; $daton=$daton;	 }
	}
	if($id_mat_prima_nacional_antiguo){ $datoi=$id_mat_prima_nacional_antiguo;	
	 
	 }
	
$sqli="SELECT mpi.id_mat_prima_importada AS id_mat_prima_importada, mpi.cruce_tablas_id as cruce_tablas_id, mpi.etiquetados_folios_id AS etiquetados_folios_id, mpi.id_c_es_so AS id_c_es_so,  mpi.id_solicitud_mp AS id_solicitud_mp, pro.producto AS producto, pro.id_producto AS id_producto, c.calibre AS calibre, c.id_calibre AS id_calibre, org.origen AS origen, org.id_origen AS id_origen, mpi.contenido AS contenido, mpi.comprobante_num AS comprobante_num, mpi.bidon_num AS bidon_num, mpi.fecha_elaboracion AS fecha_elaboracion, mpi.fecha_vencimiento AS fecha_vencimiento, mpi.fecha_ingreso AS fecha_ingreso, est.estado_material AS estado_material, c.calibre AS calibre, mpi.folio_m3_mpi as folio_m3_mpi FROM mat_prima_importada AS mpi, producto AS pro,calibre AS c, origenes AS org, estado_material AS est WHERE mpi.id_producto = pro.id_producto and mpi.id_calibre = c.id_calibre and mpi.id_origen = org.id_origen and mpi.id_estado_material = est.id_estado_material and mpi.id_estado_material != 0  and   mpi.id_estado_material = $id_estado_material ";
if($id_mat_prima_nacional){ $sqli.= " and mpi.folio_m3_mpi = '$daton' "; }
