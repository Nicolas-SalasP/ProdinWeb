...
$resultul=mysql_query($sqlul);
 ...
 $id_etiquetados_folios=$rowul[id_etiquetados_folios];
 $id_etiquetados_folios_siguiente=$id_etiquetados_folios+1;
 echo "$id_etiquetados_folios_siguiente";
 }
*/
  $dat2=split(" ",$f_elaboracion);
  $dat=split("-",$dat2[0]);
  $f_elaboracion1="$dat[2]-$dat[1]-$dat[0]";
  $f_elaboracionmes="$dat[1]";
   ...
   if ($dat[0] == 'id_mat')
   {
	...
	while ($row3=mysql_fetch_array($result3)) { 
		  ...
		  $ponderado =$valor_cmp *  $aporte_por_material;
  		  $sql_mod="insert folios_mat (id_etiquetados_folios,id_mat,contenido,valor_cmp,aporte_por_material,ponderado) values ($id_etiquetas,$id_mat,$contenido,$valor_cmp,$aporte_por_material,$ponderado)";
 ...
 $dat3=split("-",$key3); 
   if ($dat3[0] == 'id_mat3')
   {
	...
	while ($row3=mysql_fetch_array($result3)) { 
		 ...
		 $sql_mod="insert folios_mat (id_etiquetados_folios,id_mat,contenido,valor_cmp,aporte_por_material,ponderado) values ($id_etiquetas,$id_mat3,$contenido3,$valor_cmpi3,$aporte_por_material3,$ponderado3)";
    	 $result_cruce=mysql_query($sql_mod,$link);
...
//valor_unitario = '$resul_valor_unitario',
 $sqlup3="UPDATE etiquetados_folios  set total_ponderado   = '$resul_valor_unitario',  fech_generada_inicio = '$fech_generada_inicio' where id_etiquetados_folios  = $id_etiquetas";
	  $result3=mysql_query($sqlup3);   
//echo "$sqlup3<br>";

?>
<script languaje="javascript">
window.opener.document.location.replace('http://200.63.96.220/~insubac/<? echo $url;?>/sistema.php?modulo=ejemplo_etiquetas_folios.php');
</script>
